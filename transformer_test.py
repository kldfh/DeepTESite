...
encoder_number = 4  # 编码器数量
batch_size = 64  # 批次大小
# 将训练数据和标签打包成一个数据集对象
train_dataset = torch.utils.data.TensorDataset(X_train_torch, y_train_torch)
valid_dataset = torch.utils.data.TensorDataset(X_val_torch, y_val_torch)
...
def train(model, train_loader, val_loader, optimizer, criterion, num_epochs=200, patience=8):
    ...
    for epoch in range(num_epochs):
        ...
        model.train()
        # 训练模型
        for batch_idx, (X_train_torch, y_train_torch) in enumerate(train_loader):
            optimizer.zero_grad()
        ...
        val_targets = []
        with torch.no_grad():
            for batch_idx, (X_val_torch, y_val_torch) in enumerate(val_loader):
                y_val_pred = model(X_val_torch)
                val_loss += criterion(y_val_pred, y_val_torch).item()
                val_preds.extend(y_val_pred.argmax(dim=1).cpu().numpy())
                val_targets.extend(y_val_torch.cpu().numpy())
        ...
        print(
            f'Epoch {epoch + 1}/{num_epochs}, Validation Loss: {val_loss:.4f}, Accuracy: {accuracy:.4f}, F1: {f1:.4f}, MCC: {mcc:.4f}, AUROC: {auroc:.4f}, Sensitivity: {sensitivity:.4f}, Specificity: {specificity:.4f}')
...
best_model_state = train(model, train_data_loader, valid_data_loader, optimizer, criterion)
...
test_targets = []
with torch.no_grad():
    for batch_idx, (data, target) in enumerate(test_data_loader):  # 假设有test_loader用于加载测试集数据
        output = model(data)
...
print(f'Test Accuracy: {test_accuracy:.4f}, F1: {test_f1:.4f}, MCC: {test_mcc:.4f}, AUROC: {test_auroc:.4f}, Sensitivity: {test_sensitivity:.4f}, Specificity: {test_specificity:.4f}')
