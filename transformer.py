...
class MultiHeadAttentionT(nn.Module):
    ...
    def __init__(self):
        ...
        # 输入向量维度是512，每个线性层的输出维度为64，总共8个子头
        self.head = 8
        self.W_Q = nn.Linear(10, 64 * self.head)
        self.W_K = nn.Linear(10, 64 * self.head)
        self.W_V = nn.Linear(10, 64 * self.head)
        # 将维度为8*64映射回512维度，将8个子头的输出拼接
        self.linear = nn.Linear(self.head * 64, 10)
        # 对每个样本的512个特征向量进行标准化
...
class PoswiseFeedForwardNetT(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(in_channels=10, out_channels=2048, kernel_size=1)
        self.conv2 = nn.Conv1d(in_channels=2048, out_channels=10, kernel_size=1)
        self.layer_norm = nn.LayerNorm(10)
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs):
        ...
        # enc_inputs_T_expanded[:, :10] = enc_inputs
        ...
        # enc_outputs_concat = enc_outputs + enc_outputs_T
        ...
        # enc_outputs = torch.softmax(self.LinearL1(enc_outputs_concat[enc_outputs_concat.shape[0] - 1]), dim=-1)
