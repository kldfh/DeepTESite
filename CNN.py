...
model_which = 'modelCNN'  # 'model1' 'modelCNN' 'modelLSTM'
...
class ModelCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        y_pred_output = self.linear3(out)
...
with torch.no_grad():
    ...
    # fpr, tpr, thresholds = metrics.roc_curve(y_val_torch.numpy(),
    #                                          torch.max(y_predicted_imbalanced, 1)[1].numpy(), pos_label=1)
    # auc_imbalance = metrics.auc(fpr, tpr)
    # fpr, tpr, thresholds = metrics.roc_curve(y_val_eq_torch.numpy(),
    #                                          torch.max(y_predicted_balanced, 1)[1].numpy(), pos_label=1)
...
# print('modelLSTM')
modelLSTM_pred_val_imbalance = nn.Softmax(dim=1)(y_predicted_imbalanced)
modelLSTM_pred_val_balance = nn.Softmax(dim=1)(y_predicted_balanced)
...
if model_which == 'modelCNN':
    print('modelCNN')
    modelCNN_pred_val_imbalance = nn.Softmax(dim=1)(y_predicted_imbalanced)
    modelCNN_pred_val_balance = nn.Softmax(dim=1)(y_predicted_balanced)
...
with torch.no_grad():
    ...
    # fpr, tpr, thresholds = metrics.roc_curve(y_test_torch.numpy(),
    #                                          torch.max(y_predicted_test, 1)[1].numpy(), pos_label=1)
