...
def handle_data2(dataset):
    arr = []
    for line in dataset:
        line_arr = np.fromiter(line)
        arr.append(line_arr)
    return arr
...
def preprocess_data(data):
    ...
    data.rename(index=str, columns={0: "sequence"}, inplace=True)  # 给列重新命名
    return data


negative_training_set_txt = pd.read_csv('dataset/Y_data/Y_negative_train_set(81).txt', sep="\t", header=None)
print(f'读取的负样本训练集1数据：\n{negative_training_set_txt}')
positive_training_set_txt = pd.read_csv('dataset/Y_data/Y_positive_train_set(81).txt', sep="\t", header=None)
negative_test_set_txt = pd.read_csv('dataset/Y_data/Y_negative_test_set(21).txt', sep="\t", header=None)
print(f'读取的负样本训练集1数据：\n{negative_training_set_txt}')
positive_test_set_txt = pd.read_csv('dataset/Y_data/Y_positive_test_set(21).txt', sep="\t", header=None)

negative_training_set = preprocess_data(negative_training_set_txt)
...
# with open("dataset/ST_data/ST_negative_training_set_19(4308).txt", "r") as f:
#     negative_training_set_txt = f.readlines()
# negative_training_set = handle_data(negative_training_set_txt)

# with open("dataset/ST_data/ST_positive_training_set_19(4308).txt", "r") as f:
#     positive_training_set_txt = f.readlines()
# positive_training_set = handle_data(positive_training_set_txt)
# # 正负样本的测试集
# with open("dataset/ST_data/ST_negative_test_set_19(1079).txt", "r") as f:
#     negative_test_set_txt = f.readlines()
# negative_test_set = handle_data(negative_test_set_txt)
#
# with open("dataset/ST_data/ST_positive_test_set_19(1079).txt", "r") as f:
#     positive_test_set_txt = f.readlines()
# positive_test_set = handle_data(positive_test_set_txt)

# Data Preprocessing - Sequence Extraction and Labeling
# 将序列拆分成一个个的字母组成的二维列表并将同类数据拼接到一起
negative_seq_train = np.array([list(word) for word in negative_training_set])
# print(f'将序列拆分成字母列表：\n{negative_seq_train}')
# negative_seq_train = np.array(negative_seq_train, dtype=object)
# # # 正样本训练数据
positive_seq_train = np.array([list(word) for word in positive_training_set])
# positive_seq_train = np.array(positive_seq_train, dtype=object)
# print(positive_seq_train)
# # # 正负样本测试集
negative_seq_test = np.array([list(word) for word in negative_test_set])
# negative_seq_test = np.array(negative_seq_train, dtype=object)
positive_seq_test = np.array([list(word) for word in positive_test_set])
# positive_seq_test = np.array(positive_seq_test, dtype=object)
...
参数：
    ...
    dtype：用于指定数据类型
"""
negative_lab_train = np.zeros((len(negative_training_set),), dtype=int)
# negative_lab = np.zeros((negative_seq1.shape[0],), dtype=int)
positive_lab_train = np.ones((len(positive_training_set),), dtype=int)
# negative_labnew = np.zeros((negative_seqnew.shape[0],), dtype=int)
negative_lab_test = np.zeros((len(negative_test_set),), dtype=int)
# print(f'负样本测试集标签：\n{negative_lab_test}')
positive_lab_test = np.ones((len(positive_test_set),), dtype=int)

balanced_data = False
token = True
window = 33  # 3 to 19 odd number
# 正负样本训练集数据和标签
# if balanced_data:
...
start_w = 16 - int(window / 2)
end_w = 16 + int(window / 2) + 1
...
dataset_X = np.concatenate((negative_training_set, positive_training_set))
dataset_Y = np.concatenate((negative_lab_train, positive_lab_train), axis=0)
dataset_X_test = np.concatenate((negative_test_set, positive_test_set), axis=0)
dataset_Y_test = np.concatenate((negative_lab_test, positive_lab_test), axis=0)
...
dataset_X_token = dataset_X_token.reshape(len(dataset_X), 33)
...
dataset_X_token_test = dataset_X_token_test.reshape(len(dataset_X_test), 33)
...
# 超参数
num_epochs = 200  # 训练轮数，每个epoch是对整个数据集的一次完整遍历
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(Q, K, V, attn_mask):
        ...
        scores.masked_fill_(attn_mask.to('cuda'), -1e9)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self):
        ...
        self.head = 8
        self.W_Q = nn.Linear(256, 64 * self.head)
        self.W_K = nn.Linear(256, 64 * self.head)
        self.W_V = nn.Linear(256, 64 * self.head)
        # 将维度为8*64映射回512维度，将8个子头的输出拼接
        self.linear = nn.Linear(self.head * 64, 256)
        # 对每个样本的512个特征向量进行标准化
...
class MultiHeadAttentionT(nn.Module):
    ...
    def __init__(self):
        ...
        # 输入向量维度是512，每个线性层的输出维度为64，总共8个子头
        self.head = 8
        self.W_Q = nn.Linear(33, 64 * self.head)
        self.W_K = nn.Linear(33, 64 * self.head)
        self.W_V = nn.Linear(33, 64 * self.head)
        # 将维度为8*64映射回512维度，将8个子头的输出拼接
        self.linear = nn.Linear(self.head * 64, 33)
        # 对每个样本的512个特征向量进行标准化
...
class PoswiseFeedForwardNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(in_channels=256, out_channels=2048, kernel_size=1)
        self.conv2 = nn.Conv1d(in_channels=2048, out_channels=256, kernel_size=1)
        # 对每个样本的512个特征向量进行标准化
        self.layer_norm = nn.LayerNorm(256)
        # self.dropout = nn.Dropout(0.1)
...
class PoswiseFeedForwardNetT(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(in_channels=33, out_channels=2048, kernel_size=1)
        self.conv2 = nn.Conv1d(in_channels=2048, out_channels=33, kernel_size=1)
        self.layer_norm = nn.LayerNorm(33)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_inputs, enc_self_attn_mask):
        ...
        # enc_outputs_T, attn_T, multi_residual = self.enc_self_attnT(enc_inputs_T, enc_inputs_T, enc_inputs_T, enc_self_attn_mask_T)
...
class Encoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.src_emb = nn.Embedding(21, 256)
        # 位置编码情况，这里是固定的正余弦函数
        # self.pos_emb = PositionalEncoding(256)
        self.pos_emb = AAPositionalEncoding(256)
    ...
    def forward(self, enc_inputs):
        ...
        # enc_inputs_T_expanded[:, :33] = enc_inputs
        ...
        for layer in self.layers:
            ...
            # enc_outputs, enc_outputs_T, enc_self_attn, enc_self_attns_T = layer(enc_outputs, enc_outputs_T, enc_self_attn_mask, enc_self_attn_mask_T)
