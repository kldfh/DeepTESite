...
def PCA_reconstruction(origin_data):
    # print(origin_data.shape[0])
    origin_data = origin_data.cpu()
    # print("origin_dimension:", origin_data.size())
    # 将三维张量转化为三维矩阵并且降维
    reshaped_matrix_np = origin_data.detach().numpy().reshape(origin_data.shape[0], -1)
    reshaped_matrix_np_cuda = torch.Tensor(origin_data.detach().numpy().reshape(origin_data.shape[0], -1)).cuda()
    # print(reshaped_matrix_np.shape)
    # 奇异值分解
    _, _, v_t = torch.linalg.svd(reshaped_matrix_np_cuda)
    # v_t = torch.tensor(v_t).cuda()
    # print("v_t_dimension:\n", v_t.shape)
    # print("eig_value_left:", v_t)
    # 计算y_t
    y_t = torch.matmul(reshaped_matrix_np_cuda, v_t.T)
    # print("y_t_dimension:\n", y_t.shape)
    # 计算均值
    reshaped_matrix_mean = torch.Tensor(np.mean(reshaped_matrix_np, axis=0).reshape((1, reshaped_matrix_np.shape[1]))).cuda()
    # print(reshaped_matrix_mean.shape)
    # 计算x_rec
    x_rec = torch.matmul(v_t, y_t.T).T + reshaped_matrix_mean
    # 计算rec
    rec = abs(reshaped_matrix_np_cuda - x_rec)
    # print(rec.shape)
    # 转换回三维的tensor
    tensor_rec = rec.view(origin_data.shape)
    return tensor_rec
    # print(tensor_rec)
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs):
        ...
        # print("origin_enc_outputs", enc_outputs)
        enc_outputs = PCA_reconstruction(enc_outputs)
        # print("PCA_enc_outputs", enc_outputs)
